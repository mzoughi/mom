...
svg.appendChild(svgEl('text', {x:labX, y:labY-2, 'class':'ct4label'}, c.label));
svg.appendChild(svgEl('text', {x:labX, y:labY+12, 'class':'ct4val'}, c.value + ' V'));
}
...
+ '      <svg class="ct4svg' + '" id="ct4Svg' + thisq + '" viewBox="0 0 880 510" role="img" aria-label="Simplified circuit"></svg>'
...
svg.appendChild(svgEl('circle', { cx:pos[n].x, cy:pos[n].y, r:4, 'class':'ct4node' }));
});
...
function drawSkeletonWires(svg) {
svg.appendChild(svgEl('line', {x1:140, y1:50, x2:520, y2:50, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:140, y1:50, x2:140, y2:90, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:300, y1:50, x2:300, y2:90, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:520, y1:50, x2:520, y2:90, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:120, y1:360, x2:400, y2:360, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:140, y1:330, x2:140, y2:360, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:300, y1:330, x2:300, y2:360, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:520, y1:330, x2:520, y2:360, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:520, y1:360, x2:480, y2:360, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:520, y1:360, x2:520, y2:430, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:520, y1:430, x2:480, y2:430, 'class':'ct4wire'}));
svg.appendChild(svgEl('line', {x1:120, y1:430, x2:120, y2:360, 'class':'ct4wire'}));
}
function drawComponent(svg, c) {
if (c.kind === 'battery') drawBatteryDiagonal(svg, c);
else drawResistor(svg, c);
...
var px = -uy, py = ux;
if (px * (mx-320) + py * (my-240) < 0) { px = -px; py = -py; }
var labOff = 28;
var labX = mx + px*labOff, labY = my + py*labOff;
svg.appendChild(svgEl('text', {x:labX, y:labY-2, 'class':'ct4label'}, c.label));
svg.appendChild(svgEl('text', {x:labX, y:labY+12, 'class':'ct4val'}, fmt(c.value) + ' \u03a9'));
...
svg.appendChild(svgEl('line', {x1:shortCx, y1:shortCy, x2:leadEndX, y2:leadEndY, 'class':'ct4wire'}));
var pxL = px, pyL = py;
if (pxL * (mx-320) + pyL * (my-240) < 0) { pxL = -pxL; pyL = -pyL; }
var labOff = 22;
var labX = mx + pxL*labOff, labY = my + pyL*labOff;
svg.appendChild(svgEl('text', {x:labX, y:labY-2, 'class':'ct4label'}, c.label));
svg.appendChild(svgEl('text', {x:labX, y:labY+12, 'class':'ct4val'}, c.value + ' V'));
...
var ux = dx/len, uy = dy/len;
var px = -uy, py = ux;
if (px * (geom.midX - 320) + py * (geom.midY - 240) < 0) { px = -px; py = -py; }
var arrowOffset = 22;
var ax = geom.midX + px*arrowOffset, ay = geom.midY + py*arrowOffset;
var shaftLen = 28, headLen = 6, headHalfW = 4;
var tailX = ax - ux*shaftLen/2, tailY = ay - uy*shaftLen/2;
...
+ '    <div class="ct4canvasWrap' + thisq + '">'
+ '      <div class="ct4hint' + '">For each resistor, click the half where the branch current enters \u2014 that is the high-voltage end. Click the same end again to clear.</div>'
+ '      <svg class="ct4svg' + '" id="ct4Svg' + thisq + '" viewBox="0 0 700 500" role="img" aria-label="Simplified circuit"></svg>'
+ '      <div class="ct4complete' + '" id="ct4Comp' + thisq + '" role="status"></div>'
+ '    </div>'
+ '    <aside class="ct4side' + '">'
+ '      <div>'
