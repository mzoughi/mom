...
nextId: 100,
mergeCount: 0,
// STAGE 2: same topology, but R4 is replaced by a SHORT (wire).
// The R4 short merges N3 and N5 into one canonical node. As a result:
//   - R2 (N3-N4) and R3 (N4-N5=N3) form a parallel pair through N4 only,
//     with N4 dangling at degree 2 from R2 and R3 alone — the parallel-merged
//     resistor will sit on a dangling branch (no current). Pedagogically:
//     after the user merges R2||R3, that result has no path through it.
//   - R5 (N3-N6) and R1 (N6-N5=N3) become parallel between N3=N5 and N6.
//
// Resistor values (Stage 2): R1=4, R2=1, R3=3, R5=4, R6=2, R7=4, R8=2, R9=4, R10=2.
// R4 is a wire (short). Reduction:
//   R1||R5 = 2,  +R6 = 4,  ||R7 = 2,  +R8 = 4,  ||R9 = 2,  +R10 = 4.
// Final R_eq = 4 Ω, I = 2.5 A, P = 25 W.
// (Note: R2 and R3 are also reducible to R2||R3 = 3/4 Ω, but it dangles
//  from N4 and contributes nothing.)
components: [
...
{ id:'R5',  kind:'resistor', value:4, a:'N3', b:'N6', label:'R\u2085',
...
// R4: SHORT — drawn as a wire from N3 down-and-right to N5
{ id:'R4',  kind:'wire',     a:'N3', b:'N5', label:'R\u2084 (short)',
...
t.textContent = 'Resistor network with R4 shorted (replaced by wire)';
...
svg.appendChild(svgEl('text', {x:labX, y:labY+18, 'class':'ct2val'}, fmt(c.value) + ' \u03a9'));
}
...
svg.appendChild(svgEl('text', {x:labX, y:labY-2, 'class':'ct2label'}, c.label));
svg.appendChild(svgEl('text', {x:labX, y:labY+18, 'class':'ct2val'}, c.value + ' V'));
}
...
document.getElementById('ct2BtnSer' + thisq).disabled = !canTry;
document.getElementById('ct2BtnPar' + thisq).disabled = !canTry;
}
...
+ '<div class="ct2formula' + '">R = ' + formula + '</div>', 'good');
...
S.selected = [];
document.getElementById('ct2Comp' + thisq).classList.remove('ct2show');
...
var banner0 = document.getElementById('ct2Comp' + thisq);
banner0.classList.add('ct2show');
banner0.innerHTML = ''
+ '<div class="ct2compmsg' + '">No more series or parallel reductions possible, but '
...
banner.innerHTML = ''
+ '<div class="ct2compmsg' + '">'
+ '\u2713 Network reduced. R<sub>eq</sub> identified after ' + nMerges + ' merge'
+ (nMerges === 1 ? '' : 's') + '.'
+ '</div>'
+ danglingNote
+ '<div class="ct2results' + '">'
+ '<div class="ct2resultline' + '">'
+ '<span class="ct2resultlabel' + '">R<sub>eq</sub></span>'
+ '<span class="ct2resultval' + '">' + fmt(Req) + ' \u03a9</span>'
+ '</div>'
+ '<div class="ct2resultline' + '">'
+ '<span class="ct2resultlabel' + '">I</span>'
+ '<span class="ct2resultval' + '">= V / R<sub>eq</sub> = ' + fmt(V) + ' / ' + fmt(Req)
+ ' = ' + fmt(I) + ' A</span>'
+ '</div>'
+ '<div class="ct2resultline' + '">'
+ '<span class="ct2resultlabel' + '">P</span>'
+ '<span class="ct2resultval' + '">= V \u00b7 I = ' + fmt(V) + ' \u00b7 ' + fmt(I)
+ ' = ' + fmt(P) + ' W</span>'
+ '</div>'
+ '</div>'
...
detail:{ stage:2, thisq:thisq, Req:Req, I:I, P:P, V:V }
});
...
var html = ''
+ '<div class="ct2root' + '" role="region" aria-label="Bridge circuit simplification tutorial">'
...
+ '  <h3 class="ct2title' + '">Equivalent Resistance \u2014 Stage 2: R\u2084 Shorted</h3>'
...
+ '      <div class="ct2hint' + '">Click two resistors and choose Series or Parallel. Repeat until only one resistor remains \u2014 that\u2019s R<sub>eq</sub>.</div>'
+ '      <svg class="ct2svg' + '" id="ct2Svg' + thisq + '" viewBox="0 0 880 510" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Resistor network with 10 resistors and 1 battery"></svg>'
+ '      <div class="ct2complete' + '" id="ct2Comp' + thisq + '" role="status"></div>'
+ '    </div>'
+ '    <aside class="ct2side' + '" aria-label="Simplification controls">'
+ '      <div>'
+ '        <h2 class="ct2sideh' + '">Selected</h2>'
+ '        <div class="ct2selist' + '" id="ct2Sel' + thisq + '" aria-live="polite"></div>'
+ '      </div>'
+ '      <div>'
+ '        <h2 class="ct2sideh' + '">Operation</h2>'
+ '        <div class="ct2btnrow' + '">'
+ '          <button type="button" class="ct2btn' + '" id="ct2BtnSer' + thisq + '" disabled>Series</button>'
+ '          <button type="button" class="ct2btn' + '" id="ct2BtnPar' + thisq + '" disabled>Parallel</button>'
+ '        </div>'
+ '      </div>'
+ '      <div>'
+ '        <h2 class="ct2sideh' + '">Feedback</h2>'
+ '        <div class="ct2fb' + '" id="ct2Fb' + thisq + '">Find pairs of resistors in series or parallel and merge them. Keep going until only one resistor remains \u2014 that\u2019s the equivalent resistance.</div>'
+ '      </div>'
+ '      <div class="ct2btnrow' + '">'
...
+ '</div>';
root.innerHTML = html;
document.getElementById('ct2BtnSer' + thisq).addEventListener('click', trySeries);
...
applyA11y();
refreshUI();
announce('Stage 2 ready. R4 has been replaced by a short circuit. Find the new equivalent resistance.');
}
