(function(){
  ...
  function loadFromPriorStages() {
    ...
    if (!(s1 && s1.circuit)) return { ok:false, reason:'Stage 1 has not been completed yet.' };
  ...
  function checkEquation() {
    ...
    if (S.currentEqType === 'kcl') {
      // Try junction (we use the first one; KCL at the other junction is equivalent)
      canonical = canonicalKCL(S.junctions[0]);
      label = 'KCL at junction';
    } else {
    ...
    if (!areEquivalent(stuEq, canonical)) {
      // Try to give a useful diagnostic
      var diag = diagnoseError(stuEq, canonical);
      setFeedback('<strong>Not equivalent to ' + label + '.</strong> ' + diag, 'bad');
      announce('Incorrect equation. ' + diag);
      return;
    }

    // Check that this isn't a duplicate of an existing confirmed equation
  ...
  function tryAutoSolve() {
    if (S.confirmedEqs.length < expectedEqCount()) return;
    // Solve the system. We have N branches as unknowns and N equations.
    // Build the matrix A and vector b such that A x = b, where x = [I1, I2, ...].
    var N = S.branches.length;
    var currentLabels = S.branches.map(function(b){return b.label;});
    var A = [], b = [];

    // Use the first N independent equations from confirmedEqs.
    // Independence is guaranteed if each is canonically distinct (which the dup check ensures).
    var rows = S.confirmedEqs.slice(0, N);

    for (var i=0; i<N; i++) {
      var eq = rows[i].canonical;
      var row = currentLabels.map(function(lbl){return eq.coeffs[lbl] || 0;});
      // Move EMF terms to RHS
      var rhsi = -eq.rhs;
      Object.keys(eq.coeffs).forEach(function(k){
        if (currentLabels.indexOf(k) < 0) {
          rhsi -= eq.coeffs[k]; // EMF terms move to rhs
        }
      });
      A.push(row); b.push(rhsi);
    }

    var x = solveLinear(A, b);
    if (!x) {
      setFeedback('System could not be solved \u2014 your equations may not be independent. Try a different combination.', 'bad');
      return;
    }
    S.solution = {};
  ...
  function updateProgress() {
    ...
    if (n < need) note.textContent = n + ' of ' + need + ' equations confirmed';
    else note.textContent = 'All ' + need + ' equations confirmed';
  }
  ...
  function buildDOM() {
    ...
    var html = ''
      ...
      + '  <h3 class="ct5title' + '">Stage 5: Kirchhoff Equations</h3>'
  ...
  document.addEventListener('ctStageComplete', function(e){
    if (e.detail && e.detail.stage === 4 && e.detail.thisq === thisq) {
      setTimeout(reload, 0);
    }
  });
