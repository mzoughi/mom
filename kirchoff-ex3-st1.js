...
// For top-row resistors (y near 30) and bottom-row resistor (y near 470),
// force labels to be INSIDE the schematic to prevent clipping.
var px = -uy, py = ux; // perpendicular unit vector
var labOffsetMagnitude = 28;
if (my < 50) {
// Top-row resistor → push label DOWN (into schematic)
px = 0; py = 1;
} else if (my > 440) {
// Bottom-row resistor → push label UP (into schematic)
px = 0; py = -1;
} else {
// Interior — push away from layout center as before
...
svg.appendChild(svgEl('text', {x:labX, y:labY+14, 'class':'ctval'}, fmt(c.value) + ' \u03a9'));
}
